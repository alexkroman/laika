...
describe ProceduresController do
  ...
  it "should render edit template on get edit" do
    get :edit, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.procedures.first.id.to_s
    response.should render_template('procedures/edit')
  end

  it "should assign @procedure on get edit" do
    get :edit, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.procedures.first.id.to_s
    assigns[:procedure].should == @patient_data.procedures.first
  end
  ...
  it "should add an procedure on post create" do
    old_procedure_count = @patient_data.procedures.count
    post :create, :patient_data_instance_id => @patient_data.id.to_s
    @patient_data.procedures(true).count.should == old_procedure_count + 1
  end

  it "should render show partial on put update" do
    put :update, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.procedures.first.id.to_s
    response.should render_template('procedures/_show')
  end

  it "should update procedure on put update" do
    existing_procedure = @patient_data.procedures.first
    put :update, :patient_data_instance_id => @patient_data.id.to_s, :id => existing_procedure.id.to_s,
      :procedure => { :name => 'foobar' }
    existing_procedure.reload
    existing_procedure.name.should == 'foobar'
  end

  it "should remove from @patient_data.procedures on delete destroy" do
    existing_procedure = @patient_data.procedures.first
    delete :destroy, :patient_data_instance_id => @patient_data.id.to_s, :id => existing_procedure.id.to_s
    @patient_data.procedures(true).should_not include(existing_procedure)
  end
