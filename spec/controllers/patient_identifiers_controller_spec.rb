...
describe PatientIdentifiersController do
  ...
  it "should render edit template on get edit" do
    get :edit, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.patient_identifiers.first.id.to_s
    response.should render_template('patient_identifiers/edit')
  end

  it "should assign @patient_identifier on get edit" do
    get :edit, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.patient_identifiers.first.id.to_s
    assigns[:patient_identifier].should == @patient_data.patient_identifiers.first
  end
  ...
  it "should add an patient_identifier on post create" do
    old_patient_identifier_count = @patient_data.patient_identifiers.count
    post :create, :patient_data_instance_id => @patient_data.id.to_s
    @patient_data.patient_identifiers(true).count.should == old_patient_identifier_count + 1
  end

  it "should render show partial on put update" do
    put :update, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.patient_identifiers.first.id.to_s
    response.should render_template('patient_identifiers/_show')
  end

  it "should update patient_identifier on put update" do
    existing_patient_identifier = @patient_data.patient_identifiers.first
    put :update, :patient_data_instance_id => @patient_data.id.to_s, :id => existing_patient_identifier.id.to_s,
  ...
  it "should remove from @patient_data.patient_identifiers on delete destroy" do
    existing_patient_identifier = @patient_data.patient_identifiers.first
    delete :destroy, :patient_data_instance_id => @patient_data.id.to_s, :id => existing_patient_identifier.id.to_s
    @patient_data.patient_identifiers(true).should_not include(existing_patient_identifier)
  end
