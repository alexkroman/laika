...
describe ResultsController do
  ...
  before do
    ...
    @patient_data = patient_data(:jennifer_thompson)
  end
  ...
  it "should render edit template on get edit" do
    get :edit, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.all_results.first.id.to_s
    response.should render_template('results/edit')
  end

  it "should assign @result on get edit" do
    get :edit, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.all_results.first.id.to_s
    assigns[:result].should == @patient_data.all_results.first
  end
  ...
  it "should add an result on post create" do
    old_result_count = @patient_data.all_results.count
    post :create, :patient_data_instance_id => @patient_data.id.to_s
    @patient_data.all_results(true).count.should == old_result_count + 1
  end

  it "should render show partial on put update" do
    put :update, :patient_data_instance_id => @patient_data.id.to_s, :id => @patient_data.all_results.first.id.to_s
    response.should render_template('results/_show')
  end

  it "should update result on put update" do
    existing_result = @patient_data.all_results.first
    put :update, :patient_data_instance_id => @patient_data.id.to_s, :id => existing_result.id.to_s,
      :result => { :result_code_display_name => 'foobar' }
    existing_result.reload
    existing_result.result_code_display_name.should == 'foobar'
  end

  it "should remove from @patient_data.all_results on delete destroy" do
    existing_result = @patient_data.all_results.first
    delete :destroy, :patient_data_instance_id => @patient_data.id.to_s, :id => existing_result.id.to_s
    @patient_data.all_results(true).should_not include(existing_result)
  end
