...
public class UMLSValidator implements Validator
{
    ...
    private  String validateHierarchy = "SELECT count(*) as count from MRHIER where AUI = ? "+
    		"and ( PTR LIKE CONCAT('%.' , ? )  or PTR LIKE CONCAT(CONCAT( '%.' , ? ), '.%')) ";
            ...
            boolean includeInActive)
    {
        ...
        try
        {
            ...
            conn = dataSource.getConnection();
            stmt = conn.prepareStatement(validateCodeAndNameStatement);
            stmt.setString(1, getSystemName(isoSystem));
            stmt.setString(2, code);
     ...
     *      java.lang.String, java.lang.String, java.lang.String, boolean)
     */
    public boolean validateHierarchyRelationship(String isoSystem, String parent,
            String child)
    {
        int count = 0;
        String iso = getSystemName(isoSystem);
        ...
        try
        {
            String childAUI = getAUI(child, isoSystem);
            String parentAUI = getAUI(parent, isoSystem);

            conn = dataSource.getConnection();
            stmt = conn.prepareStatement(validateHierarchy);
            stmt.setString(1, childAUI);
            stmt.setString(2, parentAUI);
            stmt.setString(3, parentAUI);
            rset = stmt.executeQuery();
            if (rset.next())
            {
                count = rset.getInt("count");
    ...
    public String getAUI(String code, String system){
        
        String iso = getSystemName(system);
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rset = null;
        String retVal = null;
        try
        {
            ...
            conn = dataSource.getConnection();
            stmt = conn.prepareStatement("Select AUI as AUI from MRCONSO where CODE = ? and SAB = ?");
