...
/**
 * A validation class that will run the schematron xslt transform on a given
 * document and then check for the existence of any assert reports for the given
 * phase. If there are reports for the given phase then the validation will be
...
public class SchematronValidator extends AbstractValidator
{
    ...
    private static Namespace SVRL = Namespace.getNamespace("svrl",
    ...
    public void validate(ValidationContext context)
    {
        ...
        try
        {
            ...
            for (Iterator iterator = nodes.iterator(); iterator.hasNext();)
            {
                ...
                result.addError(el.getAttributeValue("location"), el.getChild(
                        "text", SVRL).getText(), el);
            }
